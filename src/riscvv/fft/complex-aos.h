	...
	const uint32_t column_count)
{
	...
	__epi_2xi32 ind_high  = get_ind_odd(ind_low, gvl);
    ...
    uint32_t no_jump[8];
	jump_arr4(&no_jump[0], &a_pred[0], &b_pred[0], row_count);
...
static inline void fft8xNr(
	...
	// Offsets and masks
    const uint32_t row_end = row_start + row_count;
    uint32_t a_pred[8] = {row_start <= 0, row_start <= 1, row_start <= 2, row_start <= 3, row_start <= 4, row_start <= 5, row_start <= 6, row_start <= 7};
	__epi_2xi32 t_lo_offset_r = aos8_offset_r(&a_pred[0], stride_t, gvl);
	__epi_2xi32 t_lo_offset_i = aos8_offset_i(&a_pred[0], stride_t, gvl);

    uint32_t b_pred[8] = {row_start <= 8 && row_end > 8, row_start <= 9 && row_end > 9, row_start <= 10 && row_end > 10, row_start <= 11 && row_end > 11, row_start <= 12 && row_end > 12, row_start <= 13 && row_end > 13, row_start <= 14 && row_end > 14, row_start <= 15 && row_end > 15};
	__epi_2xi32 t_hi_offset_r = aos8_offset_r(&b_pred[0], stride_t, gvl);
	__epi_2xi32 t_hi_offset_i = aos8_offset_i(&b_pred[0], stride_t, gvl);

    uint32_t no_jump[16];
	jump_arr8(&no_jump[0], &a_pred[0], &b_pred[0], row_count);

	__epi_2xi1 mask_a_r = aos8_mask_a_r(&no_jump[0], &a_pred[0], &b_pred[0], gvl);
	__epi_2xi1 mask_a_i = aos8_mask_a_i(&no_jump[0], &a_pred[0], &b_pred[0], gvl);
	__epi_2xi1 mask_b_r = aos8_mask_b_r(&no_jump[0], &a_pred[0], &b_pred[0], gvl);
	__epi_2xi1 mask_b_i = aos8_mask_b_i(&no_jump[0], &a_pred[0], &b_pred[0], gvl);
    __epi_2xi1 pg_mask;
    __epi_2xi32 ind_store = rvindex_adress(0, 2, gvl);
...
static inline void riscvv_ifft8xNr(float w[restrict static 1] ,float f[restrict static 256], size_t column_count){
	...
	__epi_2xi32 ind_index = indexN_address(gvl, 0, 2, 16, 4);
	...
	for(int column = 0; column < column_count; column+=numVals){
		...
		//load
		a_r = __builtin_epi_vload_indexed_2xf32_mask(zero, w + column * BLOCK_SIZE * 2 + 0, ind_index, pg_mask, gvl); 
        a_i = __builtin_epi_vload_indexed_2xf32_mask(zero, w + column * BLOCK_SIZE * 2 + 1, ind_index, pg_mask, gvl); 
        b_r = __builtin_epi_vload_indexed_2xf32_mask(zero, w + column * BLOCK_SIZE * 2 + BLOCK_SIZE + 0, ind_index, pg_mask, gvl); 
        b_i = __builtin_epi_vload_indexed_2xf32_mask(zero, w + column * BLOCK_SIZE * 2 + BLOCK_SIZE + 1, ind_index, pg_mask, gvl);
		...
		//store
        __builtin_epi_vstore_indexed_2xf32_mask(f + column * BLOCK_SIZE * 2 + 0, new_a_r, ind_index, pg_mask, gvl);
        __builtin_epi_vstore_indexed_2xf32_mask(f + column * BLOCK_SIZE * 2 + 1, new_a_i, ind_index, pg_mask, gvl);
        __builtin_epi_vstore_indexed_2xf32_mask(f + column * BLOCK_SIZE * 2 + BLOCK_SIZE + 0, new_b_r, ind_index, pg_mask, gvl);
        __builtin_epi_vstore_indexed_2xf32_mask(f + column * BLOCK_SIZE * 2 + BLOCK_SIZE + 1, new_b_i, ind_index, pg_mask, gvl);
	}
