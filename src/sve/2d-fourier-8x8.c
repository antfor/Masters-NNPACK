...
void print_block(float block[BLOCK_SIZE][BLOCK_SIZE]) {
	for (uint32_t row = 0; row < BLOCK_SIZE; row++) {
		for (uint32_t column = 0; column < BLOCK_SIZE; column++) {
			printf("%f ", block[row][column]);
		}
		printf("\n");
	}
}

void print_block2(float block[BLOCK_SIZE*BLOCK_SIZE]) {
	for (uint32_t row = 0; row < BLOCK_SIZE; row++) {
		for (uint32_t column = 0; column < BLOCK_SIZE; column++) {
			printf("%f ", block[row*BLOCK_SIZE + column]);
		}
		printf("\n");
	}
}
	...
	uint32_t row_offset, uint32_t column_offset)
{
	...
	float block1[BLOCK_SIZE*BLOCK_SIZE];
	float block[BLOCK_SIZE][BLOCK_SIZE];

	sve_ifft8x8_dualreal((float *) transform, transform_stride); // modifying const transform, could be a problem?
	...
	sve_ifft8x8_complex(transform, &block1[0], transform_stride); 
	...
	sve_ifft8xN_real(row0, row4, data_stride,
		row_offset, row_count,
		&block[column_offset], BLOCK_SIZE, column_count);
	*/
...
void nnp_ifft8x8_with_offset__scalar_old(
	const float transform[restrict static 1],
	float data[restrict static 1],
	size_t transform_stride, size_t data_stride,
	uint32_t row_count, uint32_t column_count,
...
void nnp_ifft8x8_with_bias__scalar_old(
	const float transform[restrict static 1],
	float data[restrict static 1],
...
void nnp_ifft8x8_with_bias__sve(
	...
	uint32_t row_count, uint32_t column_count)
{
	float *no_const_transform =  (float *) transform;
	no_const_transform[0] += *bias * 64.0f;

	nnp_ifft8x8_with_offset__sve(transform, data, transform_stride, data_stride, row_count, column_count, 0, 0);
	//nnp_ifft8x8_with_offset__scalar_old(transform, data, transform_stride, data_stride, row_count, column_count, 0, 0);
}
...
void nnp_ifft8x8_with_bias__scalar(
	...
	uint32_t row_count, uint32_t column_count)
{
	//nnp_ifft8x8_with_bias__scalar_old(transform, data, bias, transform_stride, data_stride, row_count, column_count);
	nnp_ifft8x8_with_bias__sve(transform, data, bias, transform_stride, data_stride, row_count, column_count);
...
void nnp_ifft8x8_with_offset__scalar(
	...
	uint32_t row_offset, uint32_t column_offset)
{
	nnp_ifft8x8_with_offset__scalar_old(transform, data, transform_stride, data_stride, row_count, column_count, row_offset, column_offset);
	//nnp_ifft8x8_with_offset__scalar_new(transform, data, transform_stride, data_stride, row_count, column_count, row_offset, column_offset);
}
